 ...
 * Analysis of loops
 * -----------------
 * O(1) = Doesn't contain loop, recursion and call to any other non-constant time function.
 ...
 * induction to prove to the guess is correct or incorrect.
 *
 * T(n)=2T(n/2)+n
 * let
 * T(n)=O(nlogn)
 ...
 *     <=cnlogn
 *
 * 2. Recurrence tree method - we draw a recurrence tree and calculate the time taken by
 * every level of tree. Finally, we sum the work done at all levels. The pattern is
 ...
 * Amortized Analysis
 * ------------------
 * Used for algorithms where an occasional operation is very slow, but most of the other
 * operations are faster. We analyze a sequence of operations and guarantee a worst case
 * average time which is lower than the worst case time of a partucular expensive operation.
 *
