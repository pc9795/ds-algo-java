 ...
 * Adjacency Matrix
 * ----------------
 * symmetric for undirected geeks_for_geeks.graph.
 *
 * Pros
 * ----
 ...
 *
 * To detect cycle in undirected geeks_for_geeks.graph. -> For every visited vertex ‘v’, if there is an adjacent ‘u’ such that u is
 * already visited and u is not parent of v, then there is a cycle in geeks_for_geeks.graph.
 *
 * Bipartite Graph
 * ---------------
 * A geeks_for_geeks.graph whose vertices can be divided into two independent sets, U and V such that every
 * edge (u,v) either connects a vertex from U to V or a vertex from V to U.
 *
 * A bipartite geeks_for_geeks.graph is possible if the geeks_for_geeks.graph coloring is possible using two colors such
 * that vertices in a set are colored with the same color.
 *
 * We can maintain DAG structure of a geeks_for_geeks.graph by using Topological sort.
 *
 *
 * Directed Graph
 * ==============
