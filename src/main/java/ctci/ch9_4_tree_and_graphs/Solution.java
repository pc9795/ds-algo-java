...
public class Solution {
  ...
  public static BinarySearchTree minimalTree(int[] arr) {
    return BinarySearchTree.fromBinaryTree(
        new BinaryTree().insertAtRoot(minimalTreeUtil(arr, 0, arr.length - 1)));
  }
  ...
  public static BinaryTree.BinaryTreeNode firstCommonAncestor(
      BinaryTree.BinaryTreeNode first, BinaryTree.BinaryTreeNode second) {
  ...
  /**
   * If we are not traversing the whole tree then T=O(t) size of the subtree of the first common
   * ancestor.
   */
  public static BinaryTree.BinaryTreeNode firstCommonAncestor2(
      BinaryTree.BinaryTreeNode first, BinaryTree.BinaryTreeNode second) {
  ...
  static boolean covers(
      BinaryTree.BinaryTreeNode coveringNode, BinaryTree.BinaryTreeNode coveredNode) {
  ...
  /** / t=O(n); n + n/2 + n/4 ... */
  public static BinaryTree.BinaryTreeNode firstCommonAncestorWithoutParentLinks(
      BinaryTree.BinaryTreeNode root,
      BinaryTree.BinaryTreeNode first,
  ...
  static BinaryTree.BinaryTreeNode firstCommonAncestorWithoutParentLinksUtil(
      BinaryTree.BinaryTreeNode root,
      BinaryTree.BinaryTreeNode first,
      BinaryTree.BinaryTreeNode second) {
    if (root == null) {
    ...
    return firstCommonAncestorWithoutParentLinksUtil(
        inLeft ? root.left : root.right, first, second);
  }
  ...
  public static BinaryTree.BinaryTreeNode firstCommonAncestorWithoutParentLinks2(
      BinaryTree.BinaryTreeNode root,
      BinaryTree.BinaryTreeNode first,
      BinaryTree.BinaryTreeNode second) {
    // Check the nodes in there are not
    // Null checks
    FirstCommonAncestorResult result =
        firstCommonAncestorWithoutParentLinks2Util(root, first, second);
    return result.found ? result.node : null;
  }

  static FirstCommonAncestorResult firstCommonAncestorWithoutParentLinks2Util(
      BinaryTree.BinaryTreeNode root,
      BinaryTree.BinaryTreeNode first,
      BinaryTree.BinaryTreeNode second) {
    if (root == null) {
    ...
    FirstCommonAncestorResult left =
        firstCommonAncestorWithoutParentLinks2Util(root.left, first, second);
    FirstCommonAncestorResult right =
        firstCommonAncestorWithoutParentLinks2Util(root.right, first, second);
    ...
    if (root == first || root == second) {
      // We need the FirstCommonAncestorResult because if we simply return the root then we can't
      // identify between
  ...
  private static ArrayList<LinkedList<Integer>> BSTSequencesUtil(BinaryTree.BinaryTreeNode root) {
    ...
    if (root == null) {
      // An empty one is added so that for leaf nodes the weaveLists will return a result with them
      // else it will
  ...
  private static boolean isSubtreeUtil(
      BinaryTree.BinaryTreeNode node1, BinaryTree.BinaryTreeNode node2) {
    if (node1 == null) {
      return false;
    }
  ...
  private static boolean matchTree(
      BinaryTree.BinaryTreeNode node1, BinaryTree.BinaryTreeNode node2) {
  ...
  // T=O(N*logN)(Best Case); Node at depth d is touched by nodes above it and for balanced tree the
  // depth will be logN.
  ...
  private static int pathsWithSum2Util(
      BinaryTree.BinaryTreeNode node,
      int targetSum,
      int runningSum,
