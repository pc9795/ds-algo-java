...
public class MST {
    ...
    public static UndirectedGraph kruskalMinimumSpanningTree(UndirectedGraph graph) {
        ...
        graph.edges.sort(new Comparator<GraphBase.Edge>() {
            @Override
     ...
     * we get MST for a connected gfg.graph only, so in a connected gfg.graph V=O(E) therefore O(E+V) -> O(E)
     * In case of densely connected gfg.graph E->V^2.
     *
     * @param graph
     * @return
     */
...
//                        If we implement our own gfg.heap with a map to get the position in O(1) time then it will take
//                        O(log n) time.
