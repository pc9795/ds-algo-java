...
public class Graph extends GraphBase {
  ...
  /**
   * t=O(V+E) We can't just check visited array because then it will result (1->2, 0->2) also
   * cyclic, so we just have to check for back edge which is done using recursion stack. The edge
   * that connects current vertex to the vertex in the recursion stack is a back edge. We can also
   * modify it to return all back edges.
   *
  ...
  /**
   * t=O(V+E) Only possible for a DAG. first node have in degree 0; We can look it as decreasing
  ...
  /**
   * t=O(V+E) DFS approach will not work; ex- 0->1->2->3 , dfs will call it connected but it is not.
