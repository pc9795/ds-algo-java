...
public class Applications {
  public static class BoardEntry {
    ...
    public String toString() {
      return "BoardEntry{" + "moves=" + moves + ", pos=" + pos + '}';
    }
  }

  /**
   * t=O(n); number of cells Each cell is removed and added one time only. Using BFS for unweighted
   * graph to get shortest path.
   *
   * @param n no of cells
   * @param move snack and ladder board. move[i] will be either -1 or the destination cell(either
   *     snake or ladder)
  ...
  private static void minimizeCashFlowUtil(
      int[] amount, List<Pair<Integer, Pair<Integer, Integer>>> transactions) {
    int minIndex = 0;
    int maxIndex = 0;
  ...
  public static Set<String> boggle(char matrix[][], Set<String> dict) {
    ...
    for (int i = 0; i < matrix.length; i++) {
      for (int j = 0; j < matrix[0].length; j++) {
        // This appended value will be deleted in the helper function so every time this word will
        // start with
        // a single character
  ...
  private static void boggleUtil(
      int row,
      int col,
      StringBuilder word,
