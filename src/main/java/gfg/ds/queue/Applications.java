...
public class Applications {
     ...
     * consider a example (1,1)->(2,2)->(3,3)->(1,0)->(1,2). so when we start from (1,1) and fail at (1,0) we can't
     * succeed from any point between them. Because if we start at (1,1) worst case we don't stock any extra petrol so it
     * is negligible. So by starting from any node it will result in a failure at (1,0) so we an just check from (1,2)
     * which make time complexity linear.
     *
    ...
    /**
     * t=O(1)
     * It will generate binary numbers starting from 1 till n;
     *
     * @param n numbers to generate
     * @return array of binary numbers in string.
     */
