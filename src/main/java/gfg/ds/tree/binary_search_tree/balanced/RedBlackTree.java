...
public class RedBlackTree extends BinarySearchTree {
  ...
  /** t=O(1) NOTE: It MODIFIES the links of input node. */
  ...
  private void fixInsertion(BinaryTreeNode node) {
    ...
    while (node != root && node.parent.color == BinaryTreeNode.Color.RED) {
      // We only have this condition violated for the child of root. But root is black so it is not
      // possible as
  ...
  private void fixDeletion(BinaryTreeNode toBeDeleted, BinaryTreeNode replacement) {
    if (toBeDeleted.color == BinaryTreeNode.Color.RED) {
      ...
      // If child is null or black we don't care as black height is not affected by deletion of RED
      // node.
      return;
    }
    ...
    while (toBeDeleted != root) {
      ...
      if (sibling.color == BinaryTreeNode.Color.RED) {
        // Rotation will move old sibling up; Recolor the old sibling and parent. The new sibling is
        // always black.
        ...
        if ((sibling.left == null || sibling.left.color == BinaryTreeNode.Color.BLACK)
          ...
          // Now the parent of the `toBeDeleted` is double BLACK and if it is RED we don't want to
          // progress
          ...
          if (sibling == parent.right) {
            ...
            if (sibling.right == null || sibling.right.color == BinaryTreeNode.Color.BLACK) {
              ...
              sibling.color = BinaryTreeNode.Color.RED;
              rightRotate(sibling);
              // After rotation the sibling is changed. The child of sibling takes its position and
              // this
              // turns into a Right right case.
              sibling = parent.right;
            }
            // Right right case; Sibling is at right and either both children are red or red child
            // is at right
