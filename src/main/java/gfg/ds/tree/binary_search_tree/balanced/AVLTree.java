...
public class AVLTree extends BinarySearchTree {
  ...
  /**
   * t=O(log n) In the recursive BST insert, after insertion, we get pointers to all ancestors one
   * by one in a bottom-up manner. So we don’t need parent pointer to travel up. The recursive code
  ...
  /**
   * t=O(log n) In the recursive BST delete, after deletion, we get pointers to all ancestors one by
   * one in bottom up manner. So we don’t need parent pointer to travel up. The recursive code
  ...
  private BinaryTreeNode deleteUtil(BinaryTreeNode node, int data) {
    ...
    int balanceFactor = balanceFactor(node);
    if (balanceFactor > 1) {
      // We are not comparing this with the data as in the case of insertion as the case is
      // different. Try to draw
