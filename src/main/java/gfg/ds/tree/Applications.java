...
public class Applications {
  ...
  /**
   * Can't use NryTree as while creating centroid tree sometimes the parents become children and
   * using a graph instead of updating this relationship is easy.
   */
  public static int centroidDecomposition(UndirectedGraph nryTree) {
    assert !nryTree.isEmpty();
    assert !nryTree.isCyclic()
        : "Centroid decomposition happens on a NryTree or acyclic Undirected graph";
  ...
  /** t=O(n*log n) */
  private static int createCentroidTreeUtil(
      UndirectedGraph nryTree,
      int vertex,
      Map<Integer, Integer> countMap,
      int totalCount,
      Set<Integer> centroids) {
    Optional<Integer> maybeCentroid =
        getCentroid(nryTree, vertex, countMap, totalCount, new HashSet<>(centroids));
    ...
    for (GraphBase.GraphNode neighbor : neighbours) {
      ...
      int neighbourRootedTreeTotalCount =
          getCount(nryTree, neighbor.vertex, neighbourRootedTreeCountMap, new HashSet<>(centroids));
      neighborCentroids.add(
          createCentroidTreeUtil(
              nryTree,
              neighbor.vertex,
              neighbourRootedTreeCountMap,
              neighbourRootedTreeTotalCount,
              centroids));
    }
  ...
  /** t=O(n) */
  private static Optional<Integer> getCentroid(
      UndirectedGraph nryTree,
      int vertex,
      Map<Integer, Integer> countMap,
      int totalCount,
      Set<Integer> visitedOrCentroids) {
    ...
    for (GraphBase.GraphNode neighbour : neighbours) {
      ...
      if (heaviestNeighbourVertex == -1
          || countMap.get(neighbour.vertex) > countMap.get(heaviestNeighbourVertex)) {
    ...
    return heaviestNeighbourVertex == -1
        ? Optional.empty()
        : getCentroid(nryTree, heaviestNeighbourVertex, countMap, totalCount, visitedOrCentroids);
  }
