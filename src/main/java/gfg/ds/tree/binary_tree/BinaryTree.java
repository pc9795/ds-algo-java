...
public class BinaryTree {
  ...
  /**
   * Position string is a binary string of '0s' and '1s'. 0 means left and 1 means right. So if
   * input is '01' it means that data is inserted at `root.left.right`. NOTE: You can't add the data
   * to root using this method. It is done at the time of tree initialization.
   */
  public BinaryTree insertAtPos(String pos, int data) {
    ...
    for (int i = 0; i < len - 1; i++) {
      ...
      if (ch == '0') {
        ...
        throw new RuntimeException(
            String.format("Unexpected character:%s found at position:%s", ch, i + 1));
      }
    ...
    if (pos.charAt(len - 1) == '0') {
      ...
      throw new RuntimeException(
          String.format("Unexpected character:%s found at position:%s", pos.charAt(len - 1), len));
    }
    return this;
  }
  ...
  /**
   * Position string is a binary string of '0s' and '1s'. 0 means left and 1 means right. So if
   * input is '01' it means we need data of `root.left.right`. NOTE: You can't get the data to root
   * using this method.
   */
  ...
  /** t=O(n) it will traverse the whole tree. */
  ...
  private static int heightUtil(BinaryTreeNode root, int height) {
    ...
    return Math.max(
        height + 1,
        Math.max(heightUtil(root.left, height + 1), heightUtil(root.right, height + 1)));
  }

  /**
   * t=O(n^2); for skewed tree. s=O(w); w is maximum width of tree =O(n); for perfect binary tree;
   */
  ...
  /** t=O(n) s=O(h); h is the height of the tree; due to Recursion stack =O(n); for skewed tree */
  ...
  private static void preOrderTraversalUtil(
      List<Integer> traversal, BinaryTreeNode root, boolean includeNull) {
    if (root == null) {
      if (includeNull) {
        traversal.add(null);
      }
  ...
  private static void inOrderTraversalUtil(
      List<Integer> traversal, BinaryTreeNode root, boolean includeNull) {
    if (root == null) {
      if (includeNull) {
        traversal.add(null);
  ...
  public DoublyLinkedList toList() {
    ...
    // We are getting a circular doubly list but we are returning only doubly as circular one is not
    // implemented yet.
    return dll;
  }
  ...
  public static class BinaryTreeNode {
    ...
    public BinaryTreeNode(
        int data, BinaryTreeNode left, BinaryTreeNode right, BinaryTreeNode parent) {
      this.data = data;
      this.left = left;
      this.right = right;
      this.parent = parent;
