...
public abstract class SegmentTreeBase {
  ...
  /** t=O(n) original nodes were (2*n)-1 => O(n) */
  private int build(int currIndex, int leftLimit, int rightLimit) {
    ...
    return this.val[currIndex] =
        operation(
            build(left(currIndex), leftLimit, mid), build(right(currIndex), mid + 1, rightLimit));
  }
  ...
  private int queryUtil(
    ...
    return operation(
        queryUtil(queryLeftLimit, queryRightLimit, treeLeftLimit, mid, left(currIndex)),
        queryUtil(queryLeftLimit, queryRightLimit, mid + 1, treeRightLimit, right(currIndex)));
  }
  ...
  private void updateUtil(
      int treeLeftLimit, int treeRightLimit, int index, int currIndex, int increment) {
    // Outside range
  ...
  public void updateRange(int leftLimit, int rightLimit, int increment) {
    assert leftLimit >= 0
        && rightLimit >= 0
        && leftLimit < original.length
        && rightLimit < original.length
  ...
  private void updateRangeUtil(
    ...
    updateRangeUtil(
        treeLeftLimit, mid, queryLeftLimit, queryRightLimit, left(currIndex), increment);
    updateRangeUtil(
        mid + 1, treeRightLimit, queryLeftLimit, queryRightLimit, right(currIndex), increment);
    val[currIndex] = operation(val[left(currIndex)], val[right(currIndex)]);
  }
