package test.geeks_for_geeks.ds.graph.adj_list;

import geeks_for_geeks.ds.graph.adj_list.Applications;
...
class TestGraph {
    ...
    void testAddEdgeToDAG() {
        ...
        assert Applications.addEdgeInDAG(new Edge[]{
                new Edge(3, 0), new Edge(4, 5), new Edge(0, 2)
        }, graph).equals(new HashSet<>(Arrays.asList(new Edge(0, 3), new Edge(5, 4),
                new Edge(0, 2))));
    }
    ...
    @Test
    void testIsCyclic() {
        Graph graph = new Graph(4).addEdge(0, 1).addEdge(0, 2).addEdge(1, 2)
                .addEdge(2, 0).addEdge(2, 3).addEdge(3, 3);
    ...
    void testLongestPath() {
        Graph graph = new Graph(6);
        graph.addEdge(0, 1, 5).addEdge(0, 2, 3).addEdge(1, 3, 6).
                addEdge(1, 2, 2).addEdge(2, 4, 4).addEdge(2, 5, 2)
                .addEdge(2, 3, 7).addEdge(3, 5, 1).addEdge(3, 4, -1)
                .addEdge(4, 5, -2);
        Assertions.assertArrayEquals(graph.longestPath(1), new int[]{Integer.MIN_VALUE, 0, 2, 9, 8, 10});
    }

    @Test
    void testTopologicalSort() {
        Graph g = new Graph(6).addEdge(5, 2).addEdge(5, 0).addEdge(4, 0)
    ...
    void testCheckBipartite() {
        Graph graph = new Graph(6);
        graph.addEdge(0, 1).addEdge(1, 2).addEdge(2, 3).addEdge(3, 4).addEdge(4, 5)
                .addEdge(5, 0);
    ...
    void testGetEulerPath() {
        ...
        List<Pair<Integer, Integer>> expected = Arrays.asList(new Pair<>(0, 1), new Pair<>(1, 2), new Pair<>(2, 0), new Pair<>(0, 3), new Pair<>(3, 4));
        assert graph.getEulerPath().equals(expected);

        graph.addEdge(1, 0).addEdge(0, 3).addEdge(3, 4).addEdge(4, 0).addEdge(2, 0).addEdge(1, 2);
        expected = Arrays.asList(new Pair<>(0, 1), new Pair<>(1, 2), new Pair<>(2, 0), new Pair<>(0, 3), new Pair<>(3, 4), new Pair<>(4, 0));
        assert graph.getEulerPath().equals(expected);
    }
    ...
    void testGetBridges() {
        UndirectedGraph graph = new UndirectedGraph(5);
        graph.addEdge(1, 0).addEdge(0, 3).addEdge(3, 4).addEdge(2, 0).addEdge(1, 2);
        boolean ans = GraphBase.getBridges(graph).equals(Arrays.asList(new Edge(3, 4), new Edge(0, 3)));
        assert ans;
    }
