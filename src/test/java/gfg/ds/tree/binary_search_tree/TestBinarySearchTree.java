...
class TestBinarySearchTree {
  ...
  void testFindPairWithGivenSum() {
    ...
    Pair<BinaryTree.BinaryTreeNode, BinaryTree.BinaryTreeNode> result =
        Applications.findPairWithGivenSum(bst, 33);
    assert result.equals(
        new Pair<>(new BinaryTree.BinaryTreeNode(8), new BinaryTree.BinaryTreeNode(25)));

    result = Applications.findPairWithGivenSum(bst, 24);
    assert result.equals(
        new Pair<>(new BinaryTree.BinaryTreeNode(8), new BinaryTree.BinaryTreeNode(16)));
  ...
  void testDeleteRoot() {
    bst4.delete(5);
    BinarySearchTree expected =
        new BinarySearchTree().insert(6).insert(2).insert(4).insert(1).insert(3);
  ...
  void testDeleteInternalNode() {
    bst4.delete(2);
    BinarySearchTree expected =
        new BinarySearchTree().insert(5).insert(6).insert(3).insert(1).insert(4);
    assert bst4.equals(expected);
  }
  ...
  @Test
  void testFindPreSuc() {
    Pair<BinaryTree.BinaryTreeNode, BinaryTree.BinaryTreeNode> ans = bst4.findPreSuc(1);
    assert ans.key == null;
    assert ans.value.data == 2;

    ans = bst4.findPreSuc(4);
    assert ans.equals(
        new Pair<>(new BinaryTree.BinaryTreeNode(3), new BinaryTree.BinaryTreeNode(5)));
  }
