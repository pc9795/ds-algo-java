...
The best possible time complexity achieved by first (preprocessing pattern) is O(n) and by second (preprocessing text)
is O(m) where m and n are lengths of pattern and text respectively.

# Kasai Algorithm

LCP Array is an array of size `n` (like Suffix Array). A value `lcp[i]` indicates length of the longest common prefix of
the suffixes indexed by `suffix[i]` and `suffix[i+1]`. `suffix[n-1]` is not defined as there is no suffix after it.

Let lcp of suffix beginning at `txt[i]` be `k`. If `k` is greater than `0`, then lcp for suffix beginning at `txt[i+1]`
will be at-least `k-1`. The reason is, relative order of characters remain same. If we delete the first character from
both suffixes, we know that at least `k` characters will match.
