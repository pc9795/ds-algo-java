...
One assumes that there is no need to distinguish between the different operations on the data
structure. Determines the upper bound T(n) on the total cost of a sequence of n operations, then calculates the
amortized
cost to ben T(n)/n.

# Accounting method

Early operations have an amortized cost higher than their actual cost, which accumulates a
...
When an object is pushed to the stack, it comes endowed with enough credit to pay not only for the operation of
pushing it onto the stack, but also for whatever operation will eventually remove it from gfg.stack, be that a POP, a
Multipop, or no operation at all.

# Potential method

The strategy is to define a potential function PHI which maps a state D to a scalar-valued
